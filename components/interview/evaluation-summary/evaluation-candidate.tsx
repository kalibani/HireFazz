...
const EvaluationCandidate: FC<IEvaluationCandidate | any> = ({ candidate }) => {
  const questions = candidate.result.questions;
  const scores = candidate.scores;
  const scoreNumber = candidate.scores.map((score: IScore) => score.point);
  const average = calculateAverage(scoreNumber);
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        <div className="flex flex-col gap-y-3">
          <div className="rounded-md bg-white p-3">
            <h4 className="font-semibold text-primary">Your Score Review</h4>
          </div>
        ...
        <div className="flex flex-col gap-y-3">
          <div className="rounded-md bg-white p-3">
            <h4 className="font-semibold text-primary">Your Team Review</h4>
          </div>
        </div>
        <div className="flex flex-col gap-y-3">
          <div className="rounded-md bg-white p-3">
          ...
          <div className="flex items-start justify-center gap-x-3 rounded-md bg-white p-4">
            ...
            <p className="font-semibold">Fitur akan segara hadir</p>
          </div>
        </div>
      </div>
    </>
