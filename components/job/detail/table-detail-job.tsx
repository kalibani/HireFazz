...
import { Airplay, ArrowUpDown, ChevronDown, FileSearchIcon, ShieldPlus, Trash2, Zap } from 'lucide-react'
...
export const DetailJobTable: React.FC<DetailJobTableProps> = ({ jobDetail }) => {
    ...
    const columns: ColumnDef<CvAnalysis & { cv: Cv }>[] = [
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <p className="capitalize">{row.original.cv.name}</p>
            ),
        },
        {
            accessorKey: 'addedOn',
        ...
        },
        {
            ...
            cell: ({ row }) => {
                return (
                    <p className="capitalize text-slate-400">{row.original.cv.uploadStatus}</p>
                );
            },
        },
        {
            ...
            header: ({ column }) => {
                return (
                    <Button
                        className="w-fit px-4 pl-0 hover:bg-transparent text-center"
                        variant="ghost"
                        onClick={() => column.toggleSorting(column.getIsSorted() === 'asc')}
                    >
                        Status
        ...
        },
        {
            ...
            cell: ({ row, table }) => {
                const onDelete = () => {
                    ...
                    setIsLoading(true)
                    axios.delete(`/api/cv-analysis/${row.original.id}`).finally(handleFinally)
                }
                return (
                    <div className="flex items-center gap-2">
                        <Button disabled={!!table.getSelectedRowModel().rows.length} variant="ghost" onClick={onDelete}>
    ...
    const handleAction = () => {
        ...
        if (selectedAction === 'DELETE') {
            ...
            axios.delete('/api/cv-analysis', {
                data: {
                    selectedIds: getSelectedRowIds()
                }
            }).finally(handleFinally)
        } else {
            axios.patch(`/api/cv-analysis`, { actionName: selectedAction, selectedIds: getSelectedRowIds() }).finally(handleFinally)
        }
    }
    ...
    const analyzeButton = (() => {
        ...
        const onClickAnalyze = () => {
            setIsLoading(true)
            axios.post('/api/cv-analysis/analyze', { selectedIds }).finally(handleFinally)
        }

        return (
            <Button className="flex gap-2 py-1 text-sm" disabled={!selectedIds.length} onClick={onClickAnalyze}>
                <Airplay className="size-6" />
                Analyze AI {selectedIds.length ? `(${selectedIds.length}CV selected)` : ''}
            </Button>
        )
    })()
        ...
        <div className="mt-4">
            <div className="flex justify-between">
                <div className="text-sm flex flex-col gap-2">
                    <div className="flex items-center gap-2">
                        ...
                        <p>
                            There {(cvAnalysis?.length || 0) > 1 ? 'are' : 'is'} <b>{cvAnalysis?.length} applicants</b> on{' '}
                            <b>
                                “{jobDetail?.data?.jobName}”
                            </b>
                        </p>
                    </div>
            ...
            <div className="flex items-center justify-between mt-5">
                ...
                <div className="space-x-2">
                    <Pagination itemsPerPage={perPage} totalItems={cvAnalysis?.length || 0} onPageChange={(page) => handlePagination('page', page.toString())} />
                </div>
            ...
            {isLoading && (
                <div className="fixed top-0 left-0 z-50 h-full w-full items-start justify-center rounded-lg bg-black bg-opacity-40">
                    <Loader />
                </div>
            )}
        </div>
