...
const PDFSummarizerPage = () => {
  ...
  const handleSendQuestion = async () => {
    const messages = `tell me the details of each documents?`;

    try {
      const response = await axios.post("/api/message-copy", {
        messages: messages,
        namespace: "multiple-cv",
      });
        ...
        <div className="space-y-4 mt-4">
          ...
          {files && files?.length !== 0 ? (
            <>
              ...
              <Button onClick={handleSendQuestion}>Test Question</Button>
            </>
          ) : isLoading ? (
