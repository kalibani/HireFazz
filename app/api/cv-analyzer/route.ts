...
export const POST = async (req: NextRequest) => {
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        ...
        The answer must be in the same language as the language used in this ${requirements}, for example if it is using english then answer in english, if it is using Indonesian language, answer in Indonesian language and so on.
  ...
  USER INPUT: We are looking for ${jobTitle}. Here are the requirements or qualifications or job descriptions ${requirements}`,
        },
      ],
    });
